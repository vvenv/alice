...
import { speakWord, stopSpeech } from "../lib/dictation";

type PlayState = "idle" | "playing" | "paused";

/** Pause between the two readings of the same word (classroom-style). */
...
export function usePlayback({
  ...
  /** Bumped on pause/stop/start/resume so in-flight playNextWord exits. */
  const playGenRef = useRef(0);

  // sync refs at render time for stale-closure-safe reads
  ...
  const isFinished =
    wordList.length > 0 && currentIndex >= wordList.length;
  ...
  const playNextWord = useCallback(
    async (index: number, list: string[]) => {
      ...
      // Classroom style: each word is read twice with a short gap.
