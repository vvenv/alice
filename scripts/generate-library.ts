...
/**
 * data/ 下的词表源文件 → assets/data/library.json（Flutter 侧由
 * lib/services/library_data.dart 读取）。
 *
...
for (const [category, group] of byCategory) {
  for (const entry of group) {
    items.push({
      category,
      label: entry.label,
      // `default_` 前缀是历史格式，用户存过的收藏 / 历史按它匹配，不能改。
      entry: {
        id: `default_${category}_${entry.label}`,
        text: entry.text,
        timestamp: 0,
      },
    });
...
// 打进 APK 的资源，不留缩进。
fs.writeFileSync(OUTPUT_FILE, JSON.stringify(items), "utf-8");
