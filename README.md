...
- 粘贴英文单词列表 / 拍照或相册 OCR 识别（选图后可裁切、旋转；内置智谱 GLM-4V 双档模型；支持自定义 OCR 服务商；Web 版需自备 API Key）
...
- 可调间隔、自动播放下一个；听写中可随时重听当前词（点表盘或按「重听」）
- 听写中直接调语速与「朗读中文释义」，不必退出去设置页
- 全程屏幕常亮 —— 手机放桌上写字时不会息屏
- 切后台、来电、拔耳机都会自动暂停，回来接着听，不会白读几个词
- 发音源整体失灵时会停下来说明，不会静音空转
- 键盘操作：空格暂停 · ← → 切词 · R 重听 · M 标错词 · Esc 退出
- 显示 / 隐藏当前单词，词性与释义提示
- 标记错词，本地持久化；错词本在首页菜单里可查看、划词、导出、直接开听
- 完成页给出本轮成绩（单词 / 错词 / 用时）并列出错了哪几个
- 词表删词与清空都可撤销；历史与收藏支持搜索
- 亮色 / 暗色 / 跟随系统主题；提示音与触感反馈各自可关
